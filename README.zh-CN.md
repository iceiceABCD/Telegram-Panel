...
**iceice管理工具** 用于在单个 Web 面板中统一管理和运营多个 Telegram 账号，重点覆盖账号生命周期管理、批量运营、频道/群组管理、自动化任务以及模块扩展。

现已新增**验证码和2FA分享模块**，可为客户创建独立的验证码查看链接，支持自定义API接码配置、HTML公告、联系方式等，长期稳定运营。
...
- ✨ **NEW: 验证码和2FA分享**：为客户创建独立分享链接，可查看验证码和2FA信息，支持自定义配置和长期稳定运营

## 新增功能：验证码和2FA分享模块

### 特性

- 🔗 **独立分享链接**：为每个客户生成唯一的分享URL，无需登录即可查看
- ⚙️ **自定义API配置**：集成第三方接码API，实现接码自动化
- 📢 **HTML公告定制**：添加自定义HTML公告和重要提示信息
- 📞 **灵活联系方式**：支持邮箱、电话、Telegram、微信、QQ、网站等多种联系方式
- 🔒 **安全稳定**：数据加密存储，支持分享链接启用/禁用管理
- 📊 **查看统计**：记录验证码查看状态和过期时间
